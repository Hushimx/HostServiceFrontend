...
export const CellAction: React.FC<{ data: any,setRefresh:Function }> = ({ data,setRefresh }) => {
  ...
  const handleDelete = async () => {
    try {
      const response = await fetchFromNest(`/vendor/stores/${data.id}`, {
        method: "DELETE",
      });
      ...
      if (response) {
        ...
        if (response.status === 404) {
          toast.error(t("errors.toast_delete_not_found"));
        } else {
    ...
    <>
      <DropdownMenu modal={false}>
        ...
        <DropdownMenuContent align="end">
          {/* Edit Vendor */}
          <DropdownMenuItem>
            <Link href={`/vendor/store/${data.id}`} className="w-full flex">
