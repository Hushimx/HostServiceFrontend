...
const LoginForm = () => {
  ...
  const onSubmit = async (data) => {
    try {
      const response = await fetch(process.env.NEXT_PUBLIC_API_URL + "/admin/auth/signin", {
        method: "POST",
  ...
  return (
    <>
      <Head>
        <title>{t("login.admin.login_title")}</title>
      </Head>
      ...
      >
        <div className="w-full max-w-md p-8 bg-white bg-opacity-90 text-gray-900 rounded-lg shadow-2xl">
          ...
          <h1 className="text-3xl font-bold text-center mb-6">
            {t("login.admin.login_heading")}
          </h1>
          <p className="text-center text-gray-600 mb-6">
            {t("login.admin.login_subtitle")}
          </p>
