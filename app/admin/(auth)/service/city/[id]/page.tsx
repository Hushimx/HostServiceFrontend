...
const EditServicePage: React.FC = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        await fetchFromNest(`/admin/services/cities/${id}`, {
          method: "PATCH",
          body: modifiedFields,
  ...
  useEffect(() => {
    const fetchService = async () => {
      try {
        setLoading(true);
        const service = await fetchFromNest(`/admin/services/cities/${id}`);
  ...
  return (
    <div className="w-full max-w-3xl mx-auto p-6 bg-card text-card-foreground shadow-md rounded-lg">
      <h1 className="text-2xl font-bold mb-4">{t("services_cities.edit.title")}</h1>
      <form onSubmit={formik.handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          {formik.touched.serviceId && formik.errors.serviceId && (
            <p className="text-sm text-red-500">{formik.errors.serviceId}</p>
          )}
        </div>
        ...
        <div className="flex justify-end gap-2">
          <Button variant="outline" type="button" onClick={() => router.push("/admin/service/city")}>
            {t("common.cancel")}
          </Button>
