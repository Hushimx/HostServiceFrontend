...
export default function StoresListingPage() {
  ...
  const [totalEvents, setTotalStores] = useState(0);
  ...
  useEffect(() => {
    const fetchStores = async () => {
      ...
      try {
        ...
        const response = await fetchDataTable(`/admin/events`, filters);
    ...
    fetchStores();
  }, [page, limit, title, country, city, refresh, t]);
    ...
    <PageContainer scrollable>
      <div className="space-y-4">
        ...
        <div className="flex items-start justify-between">
          <Heading
            title={`${t("events.table.title")} (${totalEvents})`}
            description={t("")}
          />
          <Link
            href={`/admin/event/add`} // Adjust to your "Add Store" route
