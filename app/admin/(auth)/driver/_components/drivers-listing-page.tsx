...
import { getColumns } from "./drivers-tables/columns"; // Dynamic columns for drivers
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetchDataTable("/admin/drivers", filters);
        setDrivers(response.data || []);
        setTotalDrivers(response.meta?.total || 0);
        setMeta({
        ...
        setError(t("errors.fetch_drivers"));
      } finally {
        setLoading(false);
      }
    };
    ...
    <PageContainer scrollable>
      <div className="space-y-4">
        <div className="flex items-start justify-between">
          <Heading
            title={`${t("drivers.table.title")} (${totalDrivers})`}
            description={""}
          />
          <Link
        ...
        {loading ? (
          ...
          <>
            <DataTable columns={columns} data={drivers}   setRefresh={setRefresh}  />
            <Pagination
