...
const Order: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAdmin = async () => {
      try {
        const data = await fetchFromNest(`/admin/orders/delivery/${orderId}`);
  ...
  const handleStatusChange = async (newStatus: OrderStatus) => {
    try {
      await fetchFromNest(`/admin/orders/delivery/${orderId}`, {
        method: "PATCH",
    ...
    <PageContainer>
      <div className="w-full max-w-5xl mx-auto p-6 bg-white rounded-md shadow-lg dark:bg-gray-800 dark:text-gray-200">
        ...
        <div className="flex justify-between items-center border-b pb-4 mb-6">
          ...
          <button
            onClick={() => router.push("/admin/orders/delivery")}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 text-base">
          {[
            { label: t("orders.info.clientName"), value: order?.clientName },
          ...
          {/* Status Dropdown */}
          <div>
            <p className="font-semibold text-gray-500 dark:text-gray-400">{t("orders.info.status")}</p>
            <select
              value={status}
        ...
        <table className="w-full text-base border-collapse border dark:border-gray-700">
          <thead className="bg-gray-100 dark:bg-gray-700 dark:text-gray-200">
            <tr>
            <th className="p-4 border dark:border-gray-600">{t("orders.info.product")}</th>
          <th className="p-4 border dark:border-gray-600">{t("orders.info.quantity")}</th>
          <th className="p-4 border dark:border-gray-600">{t("orders.info.price")}</th>
          <th className="p-4 border dark:border-gray-600">{t("orders.info.total")}</th>
            </tr>
          </thead>
          <tbody>
            {order?.orderItems.map((item) => (
              <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                ...
                <td className="p-4 border dark:border-gray-600">{order.currencySign} {item.price.toFixed(2)}</td>
                <td className="p-4 border dark:border-gray-600">{order.currencySign} {item.totalPrice.toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
