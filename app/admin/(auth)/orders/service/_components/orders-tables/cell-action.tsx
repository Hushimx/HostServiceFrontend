...
export const CellAction: React.FC<{ data: any;  setRefresh: (prev) => void }> = ({
  ...
  const fetchDrivers = async () => {
    try {
      ...
      toast.error(t("errors.fetch_drivers_error"));
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      await fetchFromNest(`/admin/orders/service/${data.id}`, {
        method: "PATCH",
        body: { status },
      });
      ...
      toast.error(t("errors.status_update_error"));
    } finally {
  ...
  const handleDriverAssignment = async () => {
    ...
    try {
      ...
      toast.success(t("success.driver_assignment_success"));
      setIsDriverModalOpen(false);
      setRefresh(prev=>prev+1); // Refresh data after success
    } catch (error) {
            toast.error(t("errors.driver_assignment_error"));
    } finally {
      setIsProcessing(false);
    }
  };
    ...
    <>
      ...
      <DropdownMenu modal={false}>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem>
            <Link
              href={`/admin/orders/service/${data.id}`}
          ...
          <DropdownMenuItem onClick={() => setIsStatusModalOpen(true)}>
            <Edit className="mr-2 h-4 w-4" /> {t("services.table.edit_status")}
          </DropdownMenuItem>
          <DropdownMenuItem
            onClick={() => {
          ...
          >
            <Truck className="mr-2 h-4 w-4" /> {t("services.table.assign_driver")}
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
      ...
      {/* Edit Status Modal */}
      <ResponsiveDialog
        title={t("services.table.edit_status_title")}
        description={t("services.table.edit_status_description")}
      ...
      >
        <div className="space-y-4">
          ...
          >
            {t("services.table.select_status")}
          </label>
          <select
            id="status"
            value={status}
          ...
          <div className="flex justify-end space-x-2 mt-4">
            ...
            <Button onClick={handleStatusUpdate} disabled={isProcessing}>
              {isProcessing ? t("common.updating") : t("common.save")}
            </Button>
          </div>
        </div>
      </ResponsiveDialog>
      ...
      {/* Assign Driver Modal */}
      <ResponsiveDialog
        title={t("services.table.assign_driver")}
        description={t("services.table.assign_driver_description")}
      ...
      >
        <div className="space-y-4">
          <label
            ...
            className="block text-sm font-medium text-gray-700 dark:text-gray-300"
          >
            {t("services.table.select_driver")}
          </label>
          <select
            id="driver"
            value={driverId}
            ...
            className="w-full p-2 border rounded-md dark:bg-gray-700 dark:text-white"
          >
            <option value="">{t("common.placeholders.select_driver")}</option>
            {drivers?.length > 0 ? (
              drivers.map((driver) => (
