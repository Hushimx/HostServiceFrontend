...
export const VendorAuthProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        const response = await axios.get(process.env.NEXT_PUBLIC_API_URL + "/vendor/auth/profile", {
        ...
        window.location.href = "/vendor/signin"; // Forces full page reload
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <VendorAuthContext.Provider value={{ user, setUser,role: user?.role }}>
      {user ? children : null}
